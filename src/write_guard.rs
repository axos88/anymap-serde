...
pub struct WriteGuard<'a, T> where T: Serialize + 'static {
  entry: Option<&'a mut SerializableAnyMapEntry>,
  ghost: PhantomData<T>,
}

impl <'a, T> WriteGuard<'a, T> where T: Serialize + 'static {
  ...
  pub fn commit(&mut self) {
    ...
    if let Some(ref mut entry) = self.entry {
      if let Some(ref boxed) = entry.value {
        let concrete_ref = boxed.downcast_ref::<T>().expect("wrong type in serialization");
  ...
  pub fn into_ref(mut self) -> &'a T {
    self.commit();
    &*self.entry.take().unwrap().value.as_ref().unwrap().downcast_ref().unwrap()
  }
}

impl<'a, T> Deref for WriteGuard<'a, T> where T: Serialize + 'static  {
  type Target = T;
  fn deref(&self) -> &T {
    self.entry
      .as_ref()
...
impl<'a, T> DerefMut for WriteGuard<'a, T> where T: Serialize + 'static {
...
impl<'a, T> Drop for WriteGuard<'a, T> where T: Serialize + 'static,
{
  fn drop(&mut self) {
    self.commit()
  }
